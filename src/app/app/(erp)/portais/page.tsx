...
export default async function Page() {
  ...
  // RBAC Guard
  if (!canAccessModule(session.role, "portais")) {
    return (
      <div className="max-w-xl mx-auto mt-12 bg-white border border-rose-200 rounded-2xl p-6 text-center space-y-4 shadow-xs">
        ...
        <p className="text-sm text-slate-600">
          O seu perfil (<strong>{session.role}</strong>) não possui permissão para acessar o módulo <strong>Módulo Portais</strong> nesta instituição escolar.
        </p>
      </div>
    );
  }
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900">Módulo Portais</h1>
            <p className="text-sm text-slate-500">Portais dedicados para Aluno, Responsável Legal e Corpo Docente.</p>
          </div>
        </div>
